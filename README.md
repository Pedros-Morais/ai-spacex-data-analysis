...
make install       # instala requirements-dev.txt (runtime + test/lint) + pacote editable
...
>
> `requirements.txt` contém só as dependências de **runtime** (usado no deploy);
> `requirements-dev.txt` adiciona as ferramentas de teste/lint.
...
3. **Em _Advanced settings_, selecione Python 3.11** (passo obrigatório — ver
   nota abaixo). O `requirements.txt` (só runtime) é detectado automaticamente.
4. *Deploy* → copie a URL pública e cole acima.

> ⚠️ **Erro comum no deploy** (`installer returned a non-zero exit code`,
> "Preparing metadata (pyproject.toml)… still running" baixando `numpy`/`pandas`
> em `.tar.gz`): o Streamlit Cloud está usando **Python 3.13**, que **não tem
> wheels** para `numpy==1.26.4`/`pandas==2.2.2` — então o pip tenta **compilar do
> código-fonte** e estoura o tempo. **Solução:** fixe **Python 3.11** em
> _Advanced settings_ (ou apague e recrie o app escolhendo 3.11). Em 3.11 todas
> as libs instalam por wheel, em segundos.
